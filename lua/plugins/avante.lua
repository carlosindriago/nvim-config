return {
  {
    ...
    opts = function(_, opts) -- Track avante's internal state during resize
      ...
      for _, tool in ipairs(tools) do
        if tool.name == "replace_in_file" then
          tool.enabled = function() return true end
        end
        if tool.name == "write_to_file" then
          tool.func = function(input, tool_opts) -- renamed opts to tool_opts to avoid conflict
            ...
            if not file then
              if tool_opts.on_complete then
                tool_opts.on_complete(false, "Failed to open file for writing: " .. tostring(err))
              end
              return false, "Failed to open file for writing: " .. tostring(err)
            end

            local ok, write_err = file:write(input.the_content)
            file:close()

            if not ok then
              if tool_opts.on_complete then
                tool_opts.on_complete(false, "Failed to write to file: " .. tostring(write_err))
              end
              return false, "Failed to write to file: " .. tostring(write_err)
            end

            if tool_opts.on_complete then
              tool_opts.on_complete(true, nil)
            end
